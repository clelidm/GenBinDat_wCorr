...
void PrintTerm_ListInteraction (list<Interaction> list_I)
{
  ...
  cout << "# 1:Op \t 2:g_Ising \t 3:g_Bin \t 4:Emp_av \t 5:Model_av" << endl;
  for (it=list_I.begin(); it!=list_I.end(); it++)
  {
    cout << "Op = " << (*it).Op << " = " << bitset<n>((*it).Op) << "\t g_Ising = " << (*it).g_Ising << "\t\t" << "\t g_Bin = " << (*it).g_Bin;
...
list<Interaction> IndepModel(map<uint32_t, unsigned int> Nset, unsigned int N, double *LogLi)
{
  ...
  cout << "# 1:Op\t 2:p1 \t 3:h_Ising \t 4:h_Bin" << endl << endl;
  ...
  for (int i=0; i<n; i++)
  {
    ...
    I.av_D = 0; I.av_M = 0;
    list_I.push_back(I); 
    cout << "Op = " << bitset<n>(I.Op) << "\t p1 = " << p1 << " \t\t h_Ising = " << I.g_Ising << " \t h_Bin = " << I.g_Bin << endl;
